...
var register_admin = exports.register_admin = function(admin,auth)
{
    return {
        ...
        create : function(req,res) {
            ...
            var method = req.body.admin_form_method.toLowerCase();
            if(method == 'put')
...
var Admin = exports.Admin = function(path,app,options)
{
    ...
    {
        res.render('admin_main.ejs',{resources:self.resources});
...
var MongooseAdminResource = exports.MongooseAdminResource = function(model,options)
{
    ...
    self.fields = null;
    options = options || {};
    self.list_fields = options.list_fields;
    self.form = function(request,options)
    {
        self.form.super_.call(this,request,options,self.model);
    };
    util.inherits(self.form,forms.MongooseForm);
...
MongooseAdminResource.prototype.index = function(req,res)
{
    ...
    self.model.find({},function(err,objects)
    {
        if(err)
            self.internal_error(err,req,res);
        else
        {
            res.render('admin_index.ejs',{objects:objects,model:self.model,list_fields:self.list_fields});
        }
    });
};
...
MongooseAdminResource.prototype.edit = function(req,res)
{
    var self = this;
    var id = req._id;
    self.model.findById(id,function(err,object)
    {
        if(err)
            self.internal_error(err,req,res);
        else
        {
            var form = new self.form(req,{instance:object});
...
//    };
    form.render_ready(function(err)
    {
        ...
        {
            context['form'] = form;
            res.render('admin_edit.ejs',context);
        }
    });
};
...
MongooseAdminResource.prototype.update = function(req,res)
{
    ...
    this.model.findById(req._id,function(err,object)
    {
        ...
        {
            var form = new self.form(req,{instance:object,data:fields});
            if(form.is_valid())
            {
...
{
    var self = this;
    var form = new this.form(req,{data:req.body});
    if(form.is_valid())
    {
